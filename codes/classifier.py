...
def getImageArray(imgPath, convertToRgb=True):
        ...
        for r in reps:
            ...
            if multiple:
                ...
                cv2.putText(imgArr, "{} @{:.2f}".format(person.decode('utf-8'), confidence),
                            (bbs[0].left(), bbs[0].bottom() + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255),
                            1)
                cv2.rectangle(imgArr, (bbs[0].left(), bbs[0].top()), (bbs[0].right(), bbs[0].bottom()),
                              (0, 255, 0), 2)
                cv2.imshow('', imgArr)
                cv2.waitKey(0)
...
if __name__ == '__main__':
    ...
    inferParser.add_argument(
        'classifierModel',
        type=str,
        help='The Python pickle representing the classifier. This is NOT the Torch '
             'network model, which can be set with --networkModel.')
